...
# Student Info
    ...
    Ned ID: qja8kc
    URL of this file on GitHub: https://github.com/bernard-gonzales/play-space/blob/main/M10-HW-1-1.Rmd 
...
```{r}
rates <- c(0.4, 0.7, 0.9, 1)
cum_payout <- c(6300, 13050, 18900, 22500)
lookup <- data.frame(rates = rates, cum_payout = cum_payout)
```
...
payout <- 
function(percent) {     
    if (percent == lookup[1, 4]) {
      return lookup$cum_payout[4]
    } else if (percent >= lookup[1, 3]) {
      return lookup$cum_payout[3] + (percent * 225000 - 202500) * 0.16
    } else if (percent >= lookup[1, 2]) {
      return lookup[2, 2] + (percent * 225000 - 157500) * 0.13
    } else if (percent >= lookup[1, 1]) {
      return lookup[2, 1] + (percent * 225000 - 90000) * 0.1
    } else {
      return percent * 225000 * 0.7
    }
}
```
...
```{r}
plot(lookup$rates, lookup$cum_payout)
```
# Task 5 
